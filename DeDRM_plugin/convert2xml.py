...
class PageParser(object):
    ...
    def getNext(self):
        nbyte = self.peek(1);
        return None if nbyte is None else readEncodedNumber(self.fo)
    ...
    def injectSnippets(self, snippet):
        ...
        tag = []
        tag.extend((name, subtagList, argtype, argList))
        snippet = []
    ...
    def formatTag(self, node):
        ...
        if len(subtagList) > 0:
            rlst.append(b'\n')
            rlst.extend(self.formatTag(j) for j in subtagList if len(j) > 0)
